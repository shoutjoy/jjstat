...
interpretation_de <- function(data, md=FALSE) {
  ...
  if ("H" %in% colnames(data)){
    ...
    for( i in 1:nrow(data)){
      rsp[i] = data[i,1]%>%str_split("->")
    }

    rsp = lapply(rsp, function(x) str_trim(x))
    Rsp = rsp %>% do.call(what= rbind)

    inter <- c()
    for(i in 1:nrow(data)) {
      # 수정된 부분: 1부터 nrow(data)까지 반복
  ...
  if ("H" %in% colnames(data)){
    ...
    for(i in 1:nrow(data)) {
      ...
      inter[i] <- paste0("가설[",i,"]: ",data[i, 1],
                         ...
                         round(data[i, 2], 3),
                         ifelse(data[i, 2] > 0,
                                paste0(" 만큼 ", ifelse(data[i, 7] == "ns", "", "유의미한 ")
                                       ,"증가를 나타낸다. "),
                                paste0("만큼 ",ifelse(data[i, 7] == "ns", "", "유의미한 "),
                                       "감소를 나타낸다. ")),
                         ifelse(data[i, 7] == "ns",
