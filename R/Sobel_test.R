...
#' }
Sobel_test <- function(a, b, sa, sb, type="all") {
  # Sobel test equation
  sobel_est = a*b
  ...
  result <- list(
    Sobel = list(sobel= type, est_ab = sobel_est,  se = sobel_se,z = sobel_z, p.value = sobel_p),
    Aroian = list(sobel=type, est_ab = aroian_est,  se = aroian_se,z = aroian_z, p.value = aroian_p),
    Goodman = list(sobel=type, est_ab = goodman_est, se = goodman_se, z = goodman_z, p.value = goodman_p)
  )

  RES =  bind_rows( Sobel = result$Sobel %>% do.call(what=bind_rows),
                    Aroian = result$Aroian%>% do.call(what=bind_rows),
                    Goodman = result$Goodman%>% do.call(what=bind_rows) )%>%
    dplyr::mutate(test=c("Sobel","Aroian","Goodman"))%>%
