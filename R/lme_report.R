...
lme_report <- function(lmedata,
 ...
 colnames(fixed_effect) = c("vars","Estimate", "Std. Error", "df", "t value", "Pr(>|t|)")

 fixed_effect <- fixed_effect %>%   p_mark_sig("Pr(>|t|)")
...
report_lme <- function(lmedata,
  ...
  #fixed effect
  fixed_effect <- lmedata_summary$coefficients %>% data.frame() %>%
    rownames_to_column("vars")
