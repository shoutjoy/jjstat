...
summary_bartlett <- function(data, test = "df", cat = TRUE) {
  ...
  if (cat) {
    ...
    if (p_value < 0.05) {
      cat("\nBartlett's 구형성 검정은 변수간에 상관관계가 있는지 확인하는 검정(test)로 유의확률(p)이 0.05이하면 변수간의 관계가 유의한 관계에 있으며, 요인분석 모형이 적합하다고 할 수 있다. 현재 모형은 유의하여 변인의 선정이 요인분석에 적절하였다,",
          result_msg_string,"\n\n")
    } else {
      cat("\nBartlett's 구형성 검정은 변수간에 상관관계가 있는지 확인하는 검정(test)로 유의확률(p)이 0.05이하면 변수간의 관계가 유의미한 관계에 있으며, 요인분석 모형이 적합하다고 할 수 있다. 현재 모형은 유의하지 않아 변인의 선정이 요인분석에 적절하지 않을 수 있다,",
          result_msg_string,
          "\n\n")
    }
  } else {
